...
if ( ! function_exists( 'crrntl_slider_settings' ) ) {
	function crrntl_slider_settings() {
		global $crrntl_slide_data, $crrntl_slider_options;
		$message = $error = '';
		...
		$action_query = add_query_arg( array(
				'page' => 'car-rental-settings',
				'tab'  => 'slider',
			),
			get_admin_url( null, 'admin.php' )
		);

		/* handle request from action links */
...
if ( ! function_exists( 'crrntl_slider_template' ) ) {
	function crrntl_slider_template() {
		if ( is_front_page() || is_home() || is_page_template( 'page-homev1.php' ) || is_page_template( 'page-homev2.php' ) || is_page_template( 'page-homev3.php' ) ) {
			global $crrntl_slider_options, $crrntl_filepath;
			if ( wp_get_theme() == 'Renty' )
				$crrntl_renty_slider_options = get_option( 'renty_options' );
...
* Add all hooks
*/
add_action( 'wp_enqueue_scripts', 'crrntl_slider_scripts' );
add_action( 'crrntl_display_slider', 'crrntl_slider_template' );
